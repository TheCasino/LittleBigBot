...
namespace LittleBigBot.Checks
{
    public class RequireDiscordContextAttribute : CheckBaseAttribute
    {
        ...
        public override Task<CheckResult> CheckAsync(ICommandContext context0, IServiceProvider provider)
        {
            ...
            return _type.HasFlag(context.Type) ? Task.FromResult(CheckResult.Successful) : Task.FromResult(new CheckResult($"This command can only be used in a {_type:G}, but we're currently in a {context.Type:G}"));
        }
    }
}