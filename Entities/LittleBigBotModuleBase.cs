...
namespace LittleBigBot.Entities
{
    ...
    {
        internal static readonly ConcurrentDictionary<ulong, IDisposable> LoggerStates =
            new ConcurrentDictionary<ulong, IDisposable>();
    }
    ...
    public abstract class LittleBigBotModuleBase : ModuleBase<LittleBigBotExecutionContext>
    {
        ...
        protected override Task BeforeExecutedAsync(Command command)
        {
            ...
            LogStateContainer.LoggerStates[Context.Invoker.Id] = NestedDiagnosticsLogicalContext.Push(
                $"Executing {command.Name} for {Context.Invoker} (ID {Context.Invoker.Id}) in {(Context.IsPrivate ? "their DM channel" : $"#{Context.Channel.Name} (ID {Context.Channel.Id})/{Context.Guild.Name} (ID {Context.Guild.Id})")} [Thread {Thread.CurrentThread.ManagedThreadId}]");
