...
namespace LittleBigBot.Parsers
{
    ...
    {
        public override async Task<TypeParserResult<T>> ParseAsync(string value, ICommandContext context0, IServiceProvider provider)
        {
            ...
            {
                if (context.Guild != null)
                    AddResult(results, await ((IGuild) context.Guild).GetUserAsync(id, CacheMode.CacheOnly).ConfigureAwait(false) as T, 1.00f);
                else
            ...
            {
                if (context.Guild != null)
                    AddResult(results, await ((IGuild) context.Guild).GetUserAsync(id, CacheMode.CacheOnly).ConfigureAwait(false) as T, 0.90f);
                else
...

            {
                await channelUsers
                    ...
                    .ForEachAsync(channelUser => AddResult(results, channelUser as T, channelUser.Username == value ? 0.65f : 0.55f))
            ...
            return results.Count > 0 ? new TypeParserResult<T>(results.Values.OrderByDescending(a => a.Value).First().Value) : new TypeParserResult<T>("User not found.");
        }
